...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        # random.choice() function will help 
        legalActions = self.getLegalActions(state)
        if len(legalActions) == 0 :
            return None

        v = self.getValue(state )
        qvs  =  [ self.getQValue( state , action  ) for action in legalActions ]
        max_actions = []
        for i,qv in enumerate( qvs  ):
            if v == qv:  max_actions.append( legalActions[i] )
    ...
    def getAction(self, state):
        ...
        return randomAction if  util.flipCoin( self.epsilon ) else policyAction 
    ...
    def update(self, state, action, nextState, reward):
        ...
        key = ( state , action  )
        self.qvalues [ key ] += self.alpha * difference   
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        return sum (  [ v * self.weights[k]   for k,v in self.featExtractor.getFeatures(state, action ).iteritems()  ] )
